...
import ipdb
import requests
from cookies import cookies, student_id # the credentials were taken from chrome inspector tool
...
'level_one': [78,79,80,81,82,83,84,85,86,87,88,89], # level one has 12 chapters
...
def get_lesson_chapter_dict(level_info):
  ...
  for chapter in chapters:
    ...
    for lesson in lessons:
      ...
      lesson_chapter_dict[lesson['id']]['chapter_name'] = ba_chapter_display_name[chapter]
...
def get_chapter_report(chapter_id):
  ...
  response = requests.post('https://beastacademy.com/api/report/getBlockResults', cookies=cookies, json=json_data)
  result = response.json()
...
def get_percent_tries_correct(completed_lesson_attempts, last_tries=3):
  ...
  for completed_lesson_attempt in completed_lesson_attempts[:last_tries]:
    ...
    qty_correct = 0
    corner_case_lesson_ids = [1546, 1469, 1470]
    if completed_lesson_attempt['setNumber'] == 'G':
      qty_correct = completed_lesson_attempt['progress']['problems'][0]['customState']['numCorrect']
      qty_questions = len(completed_lesson_attempt['progress']['problems'][0]['trials'])
      completed_lesson_attempt_scores.append(float(qty_correct) / qty_questions)
      continue
    ...
    completed_lesson_attempt_scores.append(float(qty_correct) / qty_questions)
  average_correct_last_tries = sum(completed_lesson_attempt_scores) / len(completed_lesson_attempt_scores)
...
def print_unmastered_lessons(chapter_report, min_lessons=3, mastery_percent=.9):
  lessons = chapter_report.keys()
  for lesson in lessons:
    if lesson == 'test':
      continue
    lesson_id = chapter_report[lesson]['blockID']
    lesson_name = lesson_chapter_dict[lesson_id]['displayName']
    chapter_name = lesson_chapter_dict[lesson_id]['chapter_name']
    lesson_attempted_tries = chapter_report[lesson]['results']
    completed_lesson_attempts = get_completed_lessons(lesson_attempted_tries)
...
# I'll use this code to skip any chapter, that hasn't been played at all.
# if eightyfour['students'][str(student_id)]['byBlockNumber'] == {}:
# continue
for chapter in [78, 79, 80, 81]:
  chapter_report = get_chapter_report(chapter)
  print_unmastered_lessons(chapter_report['students'][str(student_id)]['byBlockNumber'])
